...
const REVIEWER_NAME_KEY = 'dpgp-reviewer-name';
...
export default function CommentPanel({ sectionId, sectionName }: CommentPanelProps) {
  ...
  const handleAddComment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newComment.trim()) {
      // Format: "Comment text - ReviewerName"
      const commentWithName = reviewerName.trim()
        ? `${newComment.trim()} — ${reviewerName.trim()}`
        : newComment.trim();
      await addComment(sectionId, commentWithName);
      setNewComment('');
    }
  };
      ...
      >
        ...
        {comments.length > 0 && (
          <span className={`text-xs w-5 h-5 rounded-full flex items-center justify-center ${
            unresolvedCount > 0 ? 'bg-red-500 text-white' : 'bg-green-500 text-white'
          }`}>
            {comments.length}
          </span>
        )}
          ...
          <div className="max-h-64 overflow-y-auto">
            {comments.length === 0 ? (
              <div className="px-4 py-6 text-center text-gray-500 text-sm">
                ...
                No feedback yet.<br />Add your review below!
              </div>
            ) : (
